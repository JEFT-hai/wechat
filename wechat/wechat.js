	...
	return new Promise(function(resolve,reject){
	  that
	    ...
	    .then(function(data){
	    	...
	    	request({method:'POST',url:url,body:from,json:true}).then(function(response){
			var _data = response[1]
			...
			if(_data){
				...
				throw new Error('Delete msg  fails')
			}		
			})
			.catch(function(err){
				reject(err)
	  ...
	  that
	    ...
	    .then(function(data){
			...
			if(_data){
				...
				throw new Error('Preview msg  fails')
			}		
			})
			.catch(function(err){
				reject(err)
